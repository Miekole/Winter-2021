...
void PlayScene::start()
{
	...
	m_pTarget->getTransform()->position = m_getTile(5, 10)->getTransform()->position + offset;
	m_pTarget->setGridPosition(5, 10);
	addChild(m_pTarget);
...
void PlayScene::m_buildGrid()
{
	...
	// create references for each tile to its neighbors
	for (int row = 0; row < Config::ROW_NUM; ++row)
	{
		for (int col = 0; col < Config::COL_NUM; ++col)
		{
